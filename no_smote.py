# -*- coding: utf-8 -*-
"""
Created on Thu Apr 19 19:54:52 2018
...
model1 = SelectFromModel(xgb1, prefit=True,threshold=0.01)
user_info_new=DataFrame(model1.transform(user_info))
user_info_new.columns=["x_001","x_002","x_003","x_004","x_005","x_006","x_009",
                   "x_010","x_012","x_013","x_015","x_016","x_017"]

#对个人身份及财产信息生成多项式特征
poly = PolynomialFeatures(2)
poly_features=DataFrame(poly.fit_transform(user_info_new))
...
#训练xgboost模型
#设置初试参数
xgb3 = XGBClassifier(booster="gbtree",learning_rate =0.05,
                     n_estimators=800,
                     max_depth=3,
                     min_child_weight=6,
                     gamma=0,
...
gsearch1 = GridSearchCV(estimator = XGBClassifier(learning_rate =0.1, n_estimators=500, max_depth=3,
...
#调整取样比例参数   
param_test3 = {
 'subsample':[i/100.0 for i in range(70,80,2)],

}
gsearch3 = GridSearchCV(estimator = XGBClassifier( learning_rate =0.1, n_estimators=500, max_depth=3,
 min_child_weight=6, gamma=0, subsample=0.8, colsample_bytree=0.72,
 objective= 'binary:logistic', nthread=4, scale_pos_weight=4,seed=27), 
...
#调整正则式参数
param_test4 = {
 'reg_alpha':list(range(30,50,4))
}
gsearch4 = GridSearchCV(estimator = XGBClassifier( learning_rate =0.1, n_estimators=500, max_depth=3,
 min_child_weight=6, gamma=0, subsample=0.74, reg_alpha=80,colsample_bytree=0.72,
 objective= 'binary:logistic', nthread=4, scale_pos_weight=4,seed=27), 
 param_grid = param_test4, scoring='f1',n_jobs=-1,iid=False, cv=3)
gsearch4.fit(X_train,Y_train)
gsearch4.grid_scores_, gsearch4.best_params_, gsearch4.best_score_
param_test5 = {
 'scale_pos_weight':np.linspace(1,5,10)
}
gsearch5 = GridSearchCV(estimator = XGBClassifier( learning_rate =0.1, n_estimators=500, max_depth=3,
 min_child_weight=6, gamma=0, subsample=0.74, colsample_bytree=0.72,reg_alpha=42,reg_lambda=1,
...
gsearch6 = GridSearchCV(estimator = XGBClassifier( learning_rate =0.1, n_estimators=500, max_depth=3,
 min_child_weight=6, gamma=0, subsample=0.74, colsample_bytree=0.72,reg_alpha=42,reg_lambda=36,
 objective= 'binary:logistic', nthread=4, scale_pos_weight=4,seed=27), 
...
gsearch7 = GridSearchCV(estimator = XGBClassifier( learning_rate =0.1, n_estimators=500, max_depth=3,
 min_child_weight=6, gamma=0, subsample=0.74, colsample_bytree=0.72,reg_alpha=42,reg_lambda=1,
...
#调整树的数量
param_test8 = {
 'n_estimators':list(range(200,1000,50))
}
gsearch8 = GridSearchCV(estimator = XGBClassifier( learning_rate =0.05, n_estimators=500, max_depth=3,
 min_child_weight=6, gamma=0, subsample=0.74, colsample_bytree=0.72,reg_alpha=42,reg_lambda=1,
 objective= 'binary:logistic', nthread=4, scale_pos_weight=4,seed=27), 
